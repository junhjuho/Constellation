...
public class BowStringController : MonoBehaviour
{
    ...
    private void Update()
    {
        if (interactor != null)
        {
            ...
            // 활시위가 제한 거리를 초과할 경우 위치를 변경하지 않음
            if (midPointLocalZAbs > bowStringStretchLimit)
            {
                midPointLocalSpace.z = -bowStringStretchLimit;
                midPointGrabObject.position = midPointParent.TransformPoint(midPointLocalSpace);
            }
